...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            string weightage = parts[3].ToLower();
            BaseGradeBook gradeBook;
            if(type == "standard")
                gradeBook = new StandardGradeBook(name, Convert.ToBoolean(weightage));
            else if (type == "ranked")
                gradeBook = new RankedGradeBook(name,Convert.ToBoolean(weightage));
            else
            {
